...
Page({
  ...
  bindSave: function (e) {
    ...
    wx.request({
      url: app.globalData.baseUrl + '/mallPort/member/address/update/' + that.data.id,
      method: 'POST',
      header: {
...
userPhoneInput:function(e)
{
  ...
  setMorenAddress: function (e) {
    ...
    var useraddressData = {
      "city": that.data.selProvince,//城市
      "defaultStatus": 1,//是否为默认
      "detailAddress": that.data.address,//详细地址(街道)
      "id": 0,
      "name": that.data.linkMan,//收货人名称
      "phoneNumber": that.data.mobile,//收货人手机号
      "postCode": that.data.code,//邮政编码
      "province": that.data.selCity,//省份/直辖市
      "region": that.data.selDistrict//区
    };
    wx.request({
      url: app.globalData.baseUrl + '/mallPort/member/address/update/' + addressId,
      method: 'POST',
      header: {
        'content-type': 'application/json',
